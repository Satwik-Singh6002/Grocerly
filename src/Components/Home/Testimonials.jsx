...
const Testimonials = () => {
  ...
  const handleFeedbackSubmit = (e) => {
    e.preventDefault();
    if (feedback.trim()) {
      console.log("Customer Feedback:", feedback); // You can send this to backend instead
      setSubmitted(true);
      setFeedback("");
    }
  };
  ...
  return (
    <section className="py-16 bg-gradient-to-br from-green-50 to-white px-4">
      ...
      <div className="max-w-6xl mx-auto grid gap-8 md:grid-cols-3 sm:grid-cols-1">
        {testimonials.map((item, index) => (
          <div
            key={index}
            className="bg-green-100 rounded-xl p-6 shadow-md hover:shadow-xl transition-transform duration-300 hover:-translate-y-1"
          >
            <Quote className="text-green-500 mb-2 w-6 h-6" />
      ...
      <div className="mt-16 max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-lg">
        <h3 className="text-2xl font-bold text-green-700 mb-4 text-center">Leave Your Feedback</h3>
        {submitted ? (
          <p className="text-green-600 font-medium text-center">Thanks for your feedback! 💚</p>
        ) : (
          <form onSubmit={handleFeedbackSubmit} className="flex flex-col gap-4">
            <textarea
              className="w-full p-4 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500"
              rows="4"
